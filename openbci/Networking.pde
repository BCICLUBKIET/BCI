...
public void sendRawData_dataPacket(String fftdata) {
 

  //send to appropriate network type
  if (networkType == 1){
    udp.send_message(fftdata);       //Send full message to udp
  }else if (networkType == 2){
   // osc.send_message(fftdata);       //Send full message to osc
...
class UDPSend {
  ...
  void send_message(String _message){
   // String message = Arrays.toString(_message);
    udp.send(_message,ip,port);
  }
